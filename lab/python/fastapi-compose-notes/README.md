# FastAPI Compose Notes

Отдельный lab, выделенный из legacy `studyPython/homework_05`.

Что здесь полезно:

- базовое FastAPI-приложение;
- шаблоны и HTML-представления;
- `Dockerfile`;
- `docker-compose.yml`;
- `nginx.conf`.

Этот каталог стоит держать отдельно от OTUS-архива, потому что он уже ближе к промежуточному DevOps-lab, чем к чистой домашке по языку.
